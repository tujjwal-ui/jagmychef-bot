...
def feedback_trigger():
    ...
    # Use confirmed picks, or fall back to most recent history entry
    picks = [p for p in state.get("current_picks", []) if p]
    if not picks and state.get("selection_history"):
        picks = state["selection_history"][-1].get("picks", [])
    if not picks:
        return "No picks to rate yet", 200
